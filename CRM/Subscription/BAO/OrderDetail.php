 ...
 * @copyright CiviCRM LLC (c) 2004-2016
 */
class CRM_Subscription_BAO_OrderDetail extends CRM_Subscription_DAO_OrderDetail {
   ...
   *
   * @return array
   *   associated array of order details
   */
  public static function getOrderDetails($orderId) {
    $query = "
SELECT    civicrm_order_detail.id id,
          scheduled_amount,
          scheduled_date,
          reminder_date,
          reminder_count,
...
";
    ...
    $params[1] = array($orderId, 'Integer');
  ...
  public static function create($params) {
    ...
    foreach($params as $param) {
      $orderdetail = self::add($param);
      //CRM_Core_Error::debug_var('orderdetail', $orderdetail);
      if (is_a($orderdetail, 'CRM_Core_Error')) {
  ...
  public static function add($params) {
    if (!empty($params['id'])) {
      CRM_Utils_Hook::pre('edit', 'OrderDetail', $params['id'], $params);
    }
    else {
      CRM_Utils_Hook::pre('create', 'OrderDetail', NULL, $params);
    }

    $orderdetail = new CRM_Subscription_DAO_OrderDetail();
    $orderdetail->copyValues($params);

    $result = $orderdetail->save();

    if (!empty($params['id'])) {
      CRM_Utils_Hook::post('edit', 'OrderDetail', $orderdetail->id, $orderdetail);
    }
    else {
      CRM_Utils_Hook::post('create', 'OrderDetail', $orderdetail->id, $orderdetail);
    }
