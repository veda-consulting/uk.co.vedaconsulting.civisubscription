 ...
 * @copyright CiviCRM LLC (c) 2004-2016
 */
class CRM_Subscription_BAO_Order extends CRM_Subscription_DAO_Order {

  /**
   * Static field for all the order information that we can potentially export.
   *
   * @var array
   */
   ...
   *
   * @return CRM_Subscription_BAO_Order
   */
  public static function retrieve(&$params, &$defaults) {
    $order = new CRM_Subscription_DAO_Order();
    $order->copyValues($params);
    if ($order->find(TRUE)) {
      CRM_Core_DAO::storeValues($order, $defaults);
      return $order;
    }
    return NULL;
  }
  ...
  public static function add(&$params) {
    ...
    if (!isset($order->currency)) {
      $order->currency = CRM_Core_Config::singleton()->defaultCurrency;
    }

    $result = $order->save();

    if (!empty($params['id'])) {
      CRM_Utils_Hook::post('edit', 'Order', $order->id, $order);
    }
    else {
      CRM_Utils_Hook::post('create', 'Order', $order->id, $order);
    }
  ...
  public static function &getValues(&$params, &$values, $returnProperties = NULL) {
    ...
    CRM_Core_DAO::commonRetrieve('CRM_Subscription_BAO_Order', $params, $values, $returnProperties);
    return $values;
  }
  ...
  public static function create(&$params) {
    ...
    $dateFields = array('create_date', 'modified_date', 'cancel_date');
    ...
    $order = self::add($params);
    if (is_a($order, 'CRM_Core_Error')) {
      $order->rollback();
      return $order;
    }
    ...
    if (!empty($params['custom']) &&
      ...
      CRM_Core_BAO_CustomValueTable::store($params['custom'], 'civicrm_order', $order->id);
    }
    //CRM_Core_Error::debug_var('orderResult', $order);
    ...
    //CRM_Core_Error::debug_var('orderDetailsParams', $orderDetailsParams);
    CRM_Subscription_BAO_OrderDetail::create($orderDetailsParams);
  ...
  public static function deleteOrder($id) {
    CRM_Utils_Hook::pre('delete', 'Order', $id, CRM_Core_DAO::$_nullArray);

    $transaction = new CRM_Core_Transaction();

    // check for no Completed Payment records with the subscription
    $orderdetail = new CRM_Subscription_DAO_OrderDetail();
    $orderdetail->order_id = $id;
    $orderdetail->find();

    while ($orderdetail->fetch()) {
      // also delete associated contribution.
      if ($orderdetail->contribution_id) {
        CRM_Contribute_BAO_Contribution::deleteContribution($orderdetail->contribution_id);
      }
      $orderdetail->delete();
    }

    $dao = new CRM_Subscription_DAO_Order();
    $dao->id = $id;
    $results = $dao->delete();

    $transaction->commit();

    CRM_Utils_Hook::post('delete', 'Order', $dao->id, $dao);
