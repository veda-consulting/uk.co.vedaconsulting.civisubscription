...
/**
 * This class generates form components for processing a subscription
 */
class CRM_Subscription_Form_Subscription extends CRM_Core_Form {
  public $_action;

  /**
   * The id of the subscription that we are proceessing.
   *
   * @var int
   */
  public $_id;
  ...
  /**
   * The id of the contact associated with this subscription.
   *
   * @var int
   */
  public $_contactID;
  ...
  /**
   * The Subscription values if an existing subscription.
   */
  public $_values;
  ...
  /**
   * Is current subscription pending.
   */
  ...
  public function preProcess() {
    ...
    if (!CRM_Core_Permission::checkActionPermission('CiviSubscription', $this->_action)) {
    ...
    $this->setPageTitle(ts('Subscription'));

    // build custom data
    CRM_Custom_Form_CustomData::preProcess($this, NULL, NULL, 1, 'Subscription', $this->_id);
    $this->_values = array();
    // current subscription id
    if ($this->_id) {
      // get the contribution id
      $this->_contributionID = CRM_Core_DAO::getFieldValue('CRM_Subscription_DAO_SubscriptionPayment',
        $this->_id, 'contribution_id', 'subscription_id'
      );
      $params = array('id' => $this->_id);
      CRM_Subscription_BAO_Subscription::getValues($params, $this->_values);

      $this->_isPending = (CRM_Subscription_BAO_Subscription::subscriptionHasFinancialTransactions($this->_id, CRM_Utils_Array::value('status_id', $this->_values))) ? FALSE : TRUE;
    }

    // get the subscription frequency units.
  ...
  public function setDefaultValues() {
    ...
    if ($this->_id) {
      ...
      // check is this subscription pending.
    ...
    $subscriptionStatus = CRM_Subscription_BAO_Subscription::buildOptions('status_id');
    $subscriptionStatusNames = CRM_Core_OptionGroup::values('contribution_status',
      FALSE, FALSE, FALSE, NULL, 'name', TRUE
    );
    // get default status label (pending)
    $defaultSubscriptionStatus = CRM_Utils_Array::value(array_search('Pending', $subscriptionStatusNames),
      $subscriptionStatus
    );
  ...
  public function buildQuickForm() {
    ...
    foreach ($paneNames as $name => $type) {
      ...
      $allPanes[$name] = array(
        'url' => CRM_Utils_System::url('civicrm/contact/view/subscription', $urlParams),
        'open' => 'false',
        'id' => $type,
      );
    ...
    // subscription fields.
    $attributes = CRM_Core_DAO::getAttribute('CRM_Subscription_DAO_Subscription');
    ...
    $element = $this->addMoney('amount', ts('Total Subscription Amount'), TRUE,
      array_merge($attributes['subscription_amount'], $js), TRUE,
    ...
    $element = &$this->add('text', 'frequency_interval', ts('every'),
      $attributes['subscription_frequency_interval'], TRUE
    );
    ...
    if (!$this->_id || $this->_isPending) {
      $this->addDate('create_date', ts('Subscription Made'), TRUE);
    ...
    if (CRM_Utils_Array::value('status_id', $this->_values) !=
      CRM_Core_PseudoConstant::getKey('CRM_Subscription_BAO_Subscription', 'status_id', 'Cancelled')
    ) {
    ...
    $this->addFormRule(array('CRM_Subscription_Form_Subscription', 'formRule'), $this);
  ...
  public static function formRule($fields, $files, $self) {
    ...
    if ($fields['amount'] <= 0) {
      $errors['amount'] = ts('Total Subscription Amount should be greater than zero.');
    }
  ...
  public function postProcess() {
    if ($this->_action & CRM_Core_Action::DELETE) {
      CRM_Subscription_BAO_Subscription::deleteSubscription($this->_id);
      return;
    }
    ...
    if (!empty($formValues['hidden_custom'])) {
      ...
      $customFields = CRM_Core_BAO_CustomField::getFields('Subscription');
    ...
    $params['is_subscription_pending'] = $this->_isPending;

    // create subscription record.
    $subscription = CRM_Subscription_BAO_Subscription::create($params);
    ...
    if ($subscription->id) {
      ...
      if ($this->_action & CRM_Core_Action::ADD) {
        $statusMsg = ts('Subscription has been recorded and the payment schedule has been created.<br />');
      }
      elseif ($this->_action & CRM_Core_Action::UPDATE) {
        $statusMsg = ts('Subscription has been updated.<br />');
      }
    }

    // handle Acknowledgment.
    if (!empty($formValues['is_acknowledge']) && $subscription->id) {

      // calculate scheduled amount.
      $params['scheduled_amount'] = round($params['amount'] / $params['installments']);
      $params['total_subscription_amount'] = $params['amount'];
      // get some required subscription values in params.
      $params['id'] = $subscription->id;
      $params['acknowledge_date'] = $subscription->acknowledge_date;
      $params['is_test'] = $subscription->is_test;
      ...
      CRM_Subscription_BAO_Subscription::sendAcknowledgment($this, $params);
      ...
      if ($this->paymentId) {
        $urlParams = "reset=1&action=add&cid={$this->_contactID}&ppid={$this->paymentId}&context=subscription";
        ...
        if (count($processors) > 0) {
          $statusMsg .= ' ' . ts("If a payment is due now, you can record <a href='%1'>a check, EFT, or cash payment for this subscription</a> OR <a href='%2'>submit a credit card payment</a>.", array(
                1 => $contribURL,
                2 => $creditURL,
              ));
        }
        else {
          $statusMsg .= ' ' . ts("If a payment is due now, you can record <a href='%1'>a check, EFT, or cash payment for this subscription</a>.", array(1 => $contribURL));
        }
      }
    }
    ...
    if ($this->_context == 'standalone') {
      if ($buttonName == $this->getButtonName('upload', 'new')) {
        $session->replaceUserContext(CRM_Utils_System::url('civicrm/subscription/add',
          'reset=1&action=add&context=standalone'
        ));
      }
      else {
        $session->replaceUserContext(CRM_Utils_System::url('civicrm/contact/view',
          "reset=1&cid={$this->_contactID}&selectedChild=subscription"
        ));
      }
    }
    elseif ($buttonName == $this->getButtonName('upload', 'new')) {
      $session->replaceUserContext(CRM_Utils_System::url('civicrm/contact/view/subscription',
        "reset=1&action=add&context=subscription&cid={$this->_contactID}"
      ));
    }
  }
