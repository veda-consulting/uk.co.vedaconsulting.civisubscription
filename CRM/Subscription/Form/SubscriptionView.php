...
/**
 * This class generates form components for Subscription
 */
class CRM_Subscription_Form_SubscriptionView extends CRM_Core_Form {
  ...
  public function preProcess() {
    ...
    CRM_Subscription_BAO_Subscription::getValues($params,
      $values,
      $ids
    );
    ...
    if (isset($values["honor_contact_id"]) && $values["honor_contact_id"]) {
      ...
      $honor = CRM_Core_PseudoConstant::get('CRM_Subscription_DAO_Subscription', 'honor_type_id');
      $values['honor_type'] = $honor[$values['honor_type_id']];
    }

    // handle custom data.
    $groupTree = CRM_Core_BAO_CustomGroup::getTree('Subscription', $this, $params['id']);
    ...
    if ($values['status_id']) {
      $values['subscription_status'] = CRM_Core_PseudoConstant::getKey('CRM_Subscription_BAO_Subscription', 'status_id', $values['status_id']);
    }
    ...
    $title = $displayName .
      ' - (' . ts('Subscription') . ' ' . CRM_Utils_Money::format($values['subscription_amount']) .
      ' - ' . $values['financial_type'] . ')';

    // add Subscription to Recent Items
    ...
    CRM_Utils_System::setTitle(ts('View Subscription by') . ' ' . $displayName);
