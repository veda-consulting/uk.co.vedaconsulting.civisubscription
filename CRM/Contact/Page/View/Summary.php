  ...
  public function view() {
    ...
    foreach ($components as $name => $component) {
      if (!empty($this->_viewOptions[$name]) &&
        ...
        // RS Veda - Get subscription count from Subscription BAO
        $count = 0;
        if ($name == 'CiviSubscription') {
          if (CRM_Utils_File::isIncludable('CRM_Subscription_BAO_Subscription')) {
            $count = CRM_Subscription_BAO_Subscription::getContactSubscriptionCount($this->_contactId);
          }
        } else {
