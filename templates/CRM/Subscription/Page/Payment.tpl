...
{else}
{if $context eq 'dashboard'}{assign var='context' value='subscriptionDashboard'}{/if}
...
{if $context neq user}
    {if $permission EQ 'edit' and ($row.status eq 'Pending' or $row.status eq 'Overdue' or $row.status eq 'Completed') }
        ...
        {else} {* Links to record / submit a payment. *}
            ...
            {/if}
            {capture assign=editURL}{crmURL p="civicrm/subscription/payment" q="reset=1&action=update&cid=`$contactId`&context=`$context`&ppId=`$row.id`"}{/capture}
