{*
 ...
 +--------------------------------------------------------------------+
*}
{* CiviSubscription DashBoard (launch page) *}

<h3>{ts}Subscription Summary{/ts}</h3>
<table class="report">
...
<tr>
    <td scope="row"><strong>{ts}Total Subscriptions{/ts}</strong></td>
    {* prior month *}
        <td class="right"><a href="{$previousToDate.Completed.purl}">{$previousToDate.Completed.subscription_count}</a></td><td class="right" style="border-right: 5px double #999999;">{if $previousToDate.Completed.subscription_count }<a href="{$previousToDate.Completed.purl}">{$previousToDate.Completed.subscription_amount}</a>{/if}</td>
    {* current month *}
        <td class="right"><a href="{$monthToDate.Completed.purl}">{$monthToDate.Completed.subscription_count}</a></td><td class="right" style="border-right: 5px double #999999;">{if $monthToDate.Completed.subscription_count }<a href="{$monthToDate.Completed.purl}">{$monthToDate.Completed.subscription_amount}</a>{/if}</td>
    {* current year *}
        <td class="right"><a href="{$yearToDate.Completed.purl}">{$yearToDate.Completed.subscription_count}</a></td><td class="right" style="border-right: 5px double #999999;">{if $yearToDate.Completed.subscription_count }<a href="{$yearToDate.Completed.purl}">{$yearToDate.Completed.subscription_amount}</a>{/if}</td>
    {* cumulative *}
        <td class="right"><a href="{$startToDate.Completed.purl}">{$startToDate.Completed.subscription_count}</a></td><td class="right">{if $startToDate.Completed.subscription_count }<a href="{$startToDate.Completed.purl}">{$startToDate.Completed.subscription_amount}</a>{/if}</td>
</tr>
<tr>
...
{if $pager->_totalItems}
    <h3>{ts}Recent Subscriptions{/ts}</h3>
    <div>
        {include file="CRM/Subscription/Form/Selector.tpl" context="dashboard"}
    </div>
{/if}
